...
function getOpenAIClient() {
  if (!openaiClient) {
    ...
    // Fail-fast: bge-m3가 llama-swap에서 evict되면 reload(60~120s)로 L3가 막힘.
    // 15분 hang 대신 10s에 끊고 L3 빈 결과 반환(L1/L2는 계속 작동). 2026-05-25 진단 Fix.
    openaiClient  = new OpenAI({ ...options, timeout: 10000, maxRetries: 0 });
  }
  return openaiClient;
}
