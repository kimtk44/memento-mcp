...
/**
 * 쿼리에서 검색어 원본(text/keywords/topic)을 JSON 문자열로 직렬화한다.
 *
 * query_type(분류)·filter_keys(필터명)만으로는 "무엇을 검색했는지" 복원이 안 되므로,
 * 디버깅용으로 비어있지 않은 검색어 필드만 골라 보존한다. hasX 판정은
 * classifyQueryType과 동일(빈 문자열·빈 배열은 없는 것으로 처리).
 *
 * @param {{ text?: string, keywords?: string[], topic?: string }} query
 * @returns {string|null} JSON 문자열, 검색어가 전혀 없으면 null
 */
export function serializeQueryText(query) {
  if (!query || typeof query !== "object") return null;

  const picked = {};

  if (query.text !== undefined && query.text !== null && query.text !== "") {
    picked.text = query.text;
  }
  if (query.keywords !== undefined && query.keywords !== null &&
      (Array.isArray(query.keywords) ? query.keywords.length > 0 : true)) {
    picked.keywords = query.keywords;
  }
  if (query.topic !== undefined && query.topic !== null && query.topic !== "") {
    picked.topic = query.topic;
  }
...
export function buildSearchEvent(query, result, meta = {}) {
  ...
  return {
    ...
    query_text    : serializeQueryText(query),
  ...
  const sql = `
    INSERT INTO agent_memory.search_events
      (session_id, key_id, search_path, l1_count, l2_count, l3_count,
       ...
       l1_latency_ms, l2_latency_ms, l3_latency_ms, graph_used, query_text)
