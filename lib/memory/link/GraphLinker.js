...
export class GraphLinker {
  ...
  async linkFragment(fragmentId, agentId = "default", keyId = null, groupKeyIds = []) {
    ...
    /** ── Semantic Dedup Gate ──
     *  신규 파편이 명시적으로 대체(superseded_by)한 파편은 "원본"으로 보지 않는다 —
     *  대체본은 구본과 유사한 것이 정상이며, 유사도만으로 대체본을 만료시키면 안 된다. */
    ...
    const dedupResult = await queryWithAgentVector(agentId,
      `SELECT id,
       ...
       WHERE id != $1
         ...
         AND NOT EXISTS (
           SELECT 1 FROM ${SCHEMA}.fragment_links sl
            WHERE sl.from_id = ${SCHEMA}.fragments.id
              AND sl.to_id   = $1
              AND sl.relation_type = 'superseded_by'
         )
         ${dedupKeyClause}
     ...
     *  만료 파편은 제외 — 이력 파편에 related/superseded_by를 새로 걸면 supersede 체인이 오염된다. */
