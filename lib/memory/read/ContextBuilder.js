...
export class ContextBuilder {
  ...
  async #loadCoreMemory(params, agentId, keyId, groupKeyIds, workspace) {
    ...
    await Promise.all(types.map(async type => {
      ...
      if (type === "error") {
        /** [fork-patch] L0': resolved/abandoned errors must not be injected as CORE MEMORY
         *  content. buildContextHint() filters the hint signal only; the injected content
         *  line comes from here and had no resolution_status filter. NULL = open (kept). */
