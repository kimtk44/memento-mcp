...
export class FragmentReader {
   ...
   * @param {boolean} [options.includeTtlTier=false] - true면 f.ttl_tier 컬럼 추가 (forget permanent 보호용)
   */
  async searchByTopic(topic, options = {}) {
    ...
    /** forget(topic) 전용: permanent 보호 판정에 ttl_tier가 필요하다. 일반 검색 컬럼은 넓히지 않는다. */
