...
export class ReflectProcessor {
  ...
  async process(params) {
     ...
     *  [fork-patch] consolidate opt-in 시에만. 기본 off (2026-06-28 fix): sessionId는
     *  호출자가 안 넘겨도 전송 세션/세그먼트 ID로 자동 주입되고, 동시 CC 대화가 한 세션을
     *  공유할 수 있어 무조건 종합하면 타 대화 파편 혼입·기영속 파편 중복·자기참조
     *  메타요약이 생긴다. lazy auto-synthesis가 필요한 호출자만 명시 opt-in. */
    const sessionGroups = params.consolidate === true && params.sessionId
