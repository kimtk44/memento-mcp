...
export async function queryWithAgentVector(agentId, sql, params = [], opts = {}) {
  ...
  try {
     ...
     * 이 함수는 GC/CTE/DML 등 60여 비-벡터 쿼리도 거치므로 enable_seqscan=off를
     * 전역 적용하면 그들 planner를 왜곡한다. 소규모 테이블 cost 역전(17ms vs 340ms)
     * 회피는 fragments ORDER BY embedding<=>v LIMIT 경로에만 필요하다.
     */
