...
export class SessionLinker {
  ...
  async consolidateSessionFragments(sessionId, agentId = "default", keyId = null) {
    /** working-memory만 종합한다 (2026-06-28 fix): frag:sess: 영속 파편을 재방출하면
     *  이미 저장된 파편이 중복 생성된다(D3). consolidation의 목적은 미영속 WM →
     *  영속 reflect 파편 승격이지, 기영속 파편 재기록이 아니다. */
    const wmItems = await this.index.getWorkingMemory(sessionId);
    const allRows = (wmItems || []).map(w => ({
      content: w.content,
      type   : w.type || "fact"
    }));
    ...
    /** summary = 자기완결 fact의 discrete 배열 (2026-06-28 fix): 기존 "세션 …종합:" 래퍼는
     *  reflect 자체의 anti-meta 룰(자기참조 메타요약 저장 금지)을 서버가 위반하던 것이라 폐기.
     *  카운트-only fallback(`결정 N건…`)도 순수 메타라 제거. 실 내용 없으면 null. */
    const summary = summaryParts.length > 0 ? [...new Set(summaryParts)] : null;
