...
export class MemoryManager {
  ...
  async recall(params) {
    ...
    const result = await this.search.search({
      ...
      rankingMode: params.rankingMode,         // "semantic" = recency-off opt-in (기본 balanced)
     ...
     */
    const semanticMode = params.rankingMode === "semantic";
    const { importanceWeight, recencyWeight, semanticWeight, recencyHalfLifeDays } = MEMORY_CONFIG.ranking;
    result.fragments.sort((a, b) => {
      const scoreOf = (f) => {
        if (semanticMode) {
          // recency-off 순수 의미 정렬: L3 코사인 similarity 기준.
          // similarity 없는 lexical/L1 결과는 importance로 약한 tiebreak만.
          return (f.similarity || 0) + (f.importance || 0) * 0.001;
        }
        // balanced: rerankerScore(cross-encoder relevance, recencyBoost 내장) 존중.
        // 리랭크된 파편은 비-리랭크보다 위 + rerankerScore 순. 비-리랭크는 recency 복합 점수(<1.0).
        if (f.rerankerScore !== undefined) return 1000 + f.rerankerScore;
  ...
  async amend(params) {
    ...
    if (params.workspace !== undefined)       updates.workspace        = params.workspace;
