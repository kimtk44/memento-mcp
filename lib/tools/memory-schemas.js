    ...
    properties: {
      ...
      workspace: {
        ...
        description: "워크스페이스 이름 (예: 'memento-mcp', 'personal', 'client-acme'). " +
                     "프로젝트 단위 격리의 1차 경계이며, 멀티 프로젝트를 공유하는 API 키에서는 필수 기입한다. " +
                     "미지정 시 키의 default_workspace 적용. default_workspace가 없는 공유 키에서 미지정하면 다른 프로젝트 기억과 섞일 수 있다. " +
                     "전역 기억(모든 워크스페이스에서 조회)으로 저장하려면 의도적으로 지정하지 않고 키에도 default_workspace가 없어야 함.",
...
export const batchRememberDefinition = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      workspace: {
        type       : "string",
        description: "배치 기본 워크스페이스. 프로젝트 단위 격리의 1차 경계이며, 멀티 프로젝트를 공유하는 API 키로 " +
                     "일괄 저장할 때는 필수 기입한다. 개별 파편에 workspace 미지정 시 이 값으로 대체. " +
...
export const recallDefinition = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      includeContext: {
        type       : "boolean",
        description: "true이면 context_summary와 시간 인접 파편(nearby_context)을 함께 반환하고, 상위 3건에는 전후 맥락과 인과 링크를 하나로 묶은 stitched_context를 추가한다. 토큰을 더 사용하지만 전후관계 복원이 가능.",
        examples   : [true, false]
      },
      timeRange: {
        type       : "object",
      ...
      workspace: {
        ...
        description: "워크스페이스 필터. 지정 시 해당 workspace 파편 + 전역(NULL) 파편만 반환. " +
                     "미지정 시 키의 default_workspace, 그것도 없으면 전역(NULL) 파편만 반환.",
        examples   : ["memento-mcp", "personal"]
      },
      allWorkspaces: {
        type       : "boolean",
        description: "master 전용 전체 workspace 조회. true이면 workspace 필터를 제거한다. 일반 API key가 요청하면 권한 오류를 반환한다.",
        examples   : [false, true]
      },
      contextText: {
        type       : "string",
      ...
      isAnchor: {
        type       : "boolean",
        description: "앵커 필터. true=앵커만, false=비앵커만 반환하며 미지정 시 둘 다 반환.",
        examples   : [true, false]
      },
      caseMode: {
        type       : "boolean",
  ...
  inputSchema: {
    ...
    properties: {
      idempotencyKey: {
        type       : "string",
        maxLength  : 128,
        description: "재시도 안전 식별자. 같은 key_id 범위에서 같은 값으로 amend를 반복 호출하면 " +
                     "첫 호출의 응답을 그대로 반환하고 이력을 다시 쌓지 않는다. " +
                     "네트워크 단절로 응답을 받지 못한 클라이언트가 다시 호출할 때 중복 처리를 막는다.",
        examples   : ["amend-2026-08-28-001"]
      },
      id: {
        type       : "string",
      ...
      targetWorkspace: {
        type       : "string",
        description: "[fork-patch] 파편을 이전할 대상 워크스페이스. 명시 시 해당 workspace로 변경(workspace_source=explicit), " +
                     "미명시 시 기존 workspace 보존. (recall의 workspace 필터와 충돌 방지를 위해 별도 이름 사용)",
        examples   : ["personal", "work", "sandbox"]
      },
      agentId: COMMON_PARAMS.agentId,
      dryRun: {
        type       : "boolean",
...
export const reflectDefinition = {
  ...
  description : "세션 종료 시 학습 내용을 원자 파편으로 영속화한다. " +
                 ...
                 "sessionId와 consolidate=true를 함께 전달하면 해당 세션의 working-memory로 미입력 항목을 자동 채운다. " +
  ...
  inputSchema: {
    ...
    properties: {
      idempotencyKey: {
        type       : "string",
        maxLength  : 128,
        description: "재시도 안전 식별자. 같은 key_id 범위에서 같은 값으로 reflect를 반복 호출하면 " +
                     "첫 호출이 만든 파편 목록을 그대로 반환하고 다시 저장하지 않는다. " +
                     "네트워크 단절로 응답을 받지 못한 클라이언트가 다시 호출할 때 중복 처리를 막는다.",
        examples   : ["reflect-2026-08-28-001"]
      },
      summary: {
        oneOf: [
          { type: "string" },
      ...
      sessionId: {
        type       : "string",
        description: "세션 ID. 전달 시 episode preceded_by 엣지 생성 + (consolidate=true일 때) 해당 세션 working-memory 자동 종합 대상"
      },
      consolidate: {
        type       : "boolean",
        description: "[fork-patch] true일 때만 해당 세션의 working-memory에서 미입력 항목(summary/decisions/...)을 자동 종합한다. " +
                     "기본 false. sessionId는 전송 세션/세그먼트 ID로 자동 주입되고 동시 실행 중인 다른 대화와 한 세션을 공유할 수 있어, " +
                     "무조건 종합 시 타 대화 파편 혼입·기영속 파편 중복을 유발한다. lazy auto-synthesis가 필요한 경우만 명시 opt-in."
      },
      decisions: {
        type       : "array",
        items      : { type: "string" },
      ...
      workspace: {
        type       : "string",
        description: "생성되는 모든 reflect 파편에 적용할 워크스페이스. 프로젝트 단위 격리의 1차 경계이며, " +
                       "멀티 프로젝트를 공유하는 API 키에서는 필수 기입한다. " +
                       "미지정 시 API 키의 default_workspace, 그것도 없으면 전역(NULL)으로 저장된다. " +
                       "미기입 시 세션 요약이 다른 프로젝트 context에 주입되는 혼입이 발생할 수 있다.",
        examples   : ["memento-mcp", "personal"]
      },
      agentId: {
        type       : "string",
      ...
      task_effectiveness: {
        ...
        properties : {
          overall_success: {
            type       : "boolean",
            description: "호환 유지 필드. outcome을 함께 보내면 outcome이 우선한다."
          },
          outcome: {
            type       : "string",
            enum       : ["completed", "partial", "blocked", "abandoned", "unknown"],
            description: "작업 종료 상태. completed=요구사항 전부 충족, partial=일부만 충족, " +
                           "blocked=외부 요인으로 진행 불가, abandoned=중단, unknown=판정 불가. " +
                           "확신이 없으면 추정하지 말고 unknown을 쓸 것.",
            examples   : ["completed", "partial"]
          },
          evaluator: {
            type       : "string",
            enum       : ["agent", "automatic", "human"],
            description: "outcome 판정 주체. agent=에이전트 자기보고, " +
                           "automatic=테스트·빌드 등 자동 판정, human=사용자가 직접 확인. " +
                           "생략 시 agent로 기록된다.",
            examples   : ["agent", "automatic"]
          },
          evidence: {
            type       : "string",
            description: "outcome 판정 근거 (선택, 1000자 이내). 통과한 테스트 수, 확인한 로그, 사용자 승인 발화 등.",
            examples   : ["npm test 412건 전건 통과 후 사용자가 결과를 확인함"]
          },
          unmet_requirements: {
            type       : "array",
            items      : { type: "string" },
            description: "충족하지 못한 요구사항 목록 (선택, 최대 20건·각 200자). " +
                           "partial/blocked/abandoned일 때 무엇이 남았는지 명시한다.",
            examples   : [["Redis 클러스터 페일오버 검증 미완료"]]
          },
          tool_highlights: {
            type       : "array",
            items      : { type: "string" },
...
export const contextDefinition = {
  ...
  description : "Core Memory + Working Memory + session_reflect를 분리 로드한다. " +
                 ...
                 "응답은 ANCHOR/CORE/LEARNING/WORKING MEMORY 섹션과 선택적 SYSTEM HINT를 제공한다. " +
                 "flat/structured 모두 같은 중복 제거·토큰 선택 결과를 사용한다. " +
  ...
  inputSchema: {
    ...
    properties: {
      tokenBudget: {
        type       : "number",
        description: "주입 토큰 목표치 (기본 2000). 앵커와 core 유형별/learning/working 최소 1건은 맥락 소실 방지를 위해 우선 보장되므로 합계가 목표치를 넘을 수 있으며, 나머지 후보는 낮은 점수부터 제외된다.",
      ...
      allWorkspaces: {
        type       : "boolean",
        description: "master 전용 전체 workspace 조회. true이면 anchor/core/learning/working memory의 workspace 필터를 제거한다. 일반 API key가 요청하면 권한 오류를 반환한다.",
        examples   : [false, true]
      },
      structured: {
        type       : "boolean",
  ...
  inputSchema: {
    ...
    properties: {
      idempotencyKey: {
        type       : "string",
        maxLength  : 128,
        description: "재시도 안전 식별자. 같은 key_id 범위에서 같은 값으로 tool_feedback를 반복 호출하면 " +
                     "첫 호출의 응답을 그대로 반환하고 링크 가중치를 다시 움직이지 않는다. 네트워크 단절로 응답을 받지 못한 클라이언트가 " +
                     "다시 호출할 때 중복 처리를 막는다. 권장 형식: {작업명}-{날짜}-{순번}",
        examples   : ["tool_feedback-2026-08-28-001"]
      },
      tool_name: {
        type       : "string",
      ...
      irrelevance_reason: {
        type       : "string",
        enum       : ["not_stored", "search_miss", "scope_leak", "topic_mismatch", "other"],
        description: "relevant=false일 때만 의미를 갖는다. " +
                     "not_stored=저장된 적 없음, search_miss=저장됐으나 미검색, " +
                     "scope_leak=타 스코프 유입, topic_mismatch=주제 불일치, other=그 외. " +
                     "원인별 분포가 검색 품질 개선 지점을 특정한다.",
        examples   : ["search_miss", "not_stored"]
      },
      search_event_id: {
        type       : "integer",
    ...
    properties: {
      section: {
        ...
        description: "특정 섹션만 조회. 미지정 시 전체 가이드 반환. " +
                     "가능한 값: overview, lifecycle, keywords, search, episode, multiplatform, codex, tools, importance, experiential, cbr, triggers, workspace, antipatterns",
