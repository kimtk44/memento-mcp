...
export async function queryWithAgentVector(agentId, sql, params = [], opts = {}) {
  ...
  try {
    ...
    if (safeAgent !== "system" && safeAgent !== "admin") {
      const stmtTimeout = Number(process.env.DB_STATEMENT_TIMEOUT_MS || 30000);
      if (stmtTimeout > 0) {
    ...
    if (opts.forceVectorIndex && process.env.MEMENTO_VECTOR_FORCE_INDEX !== "off") {
      ...
      /**
       * [fork-patch] hnsw.iterative_scan은 pgvector 0.8.0+ 전용.
       * 0.6.0에서 이 SET은 "invalid configuration parameter name"으로 트랜잭션을
       * 중단시켜 L3 시맨틱 검색 전체를 무력화한다. 명시 opt-in일 때만 적용.
       */
