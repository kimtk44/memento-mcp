 ...
 *
 * 표집되지 않으면 응답을 그대로 반환하여 기존 응답 형태를 건드리지 않는다.
 * 표집된 경우에만 recall/context 와 동일한 `_meta` 블록을 덧붙인다.
 *
 ...
 * @returns {Promise<Object>}
 */
async function withFeedbackMeta(toolName, sessionId, response) {
  try {
    const { maybeFeedbackHint } = await import("../memory/signals/FeedbackSampler.js");
    const hint = await maybeFeedbackHint(toolName, sessionId);
    ...
    /** fail-open: 힌트 부착 실패가 도구 결과를 가려서는 안 된다. */
    return response;
  }
}
...
export async function tool_remember(args) {
  ...
  try {
    ...
    /** dryRun은 실제 저장이 아니므로 피드백을 요청하지 않는다. */
    if (args.dryRun) return { success: true, ...result };
    return withFeedbackMeta("remember", sessionId, { success: true, ...result });
  } catch (err) {
...
export async function tool_forget(args) {
  ...
  return withAudit("forget", args, async (a, sessionId) => {
    ...
    /** dryRun은 실제 삭제가 아니므로 피드백을 요청하지 않는다. */
    const response = a.dryRun
      ? { success: true, ...result }
      : await withFeedbackMeta("forget", sessionId, { success: true, ...result });
    return {
      response,
      auditOnSuccess: {
        fragmentId: a.id    || "-",
...
export async function tool_amend(args) {
  ...
  return withAudit("amend", args, async (a, sessionId) => {
    ...
    const amended  = { success: result.updated, ...result };
    /** 실제 갱신이 일어난 경우에만 피드백을 요청한다. */
    const response = result.updated === true
      ? await withFeedbackMeta("amend", sessionId, amended)
      : amended;
    return {
      response,
      auditOnSuccess: {
...
export async function tool_memoryStats(args) {
  ...
  try {
    ...
    const {
      computeRollingPrecision,
      computeTaskSuccessRate,
      computeIrrelevanceBreakdown
    } = await import("../memory/signals/EvaluationMetrics.js");
    const [evaluation, taskSuccess, irrelevance, searchObs] = await Promise.all([
      ...
      computeIrrelevanceBreakdown(30).catch(() => null),
    ...
    return {
      ...
      stats: {
        ...
        evaluation: {
          ...
          task_completed_rate   : taskSuccess.completed_rate ?? null,
          task_outcome_reported : taskSuccess.outcome_reported_sessions ?? 0,
          task_outcome_counts   : taskSuccess.outcome_counts ?? null,
          irrelevance_breakdown : irrelevance
        },
 ...
 *
 * @param {Object[]} fragments       - 응답 프로젝션이 끝난 파편 배열
 * @param {Object}   args            - tool_recall 인자
 * @param {Object}   [flags]         - { contradictionPending }
 * @param {Object[]} [topicCandidates] - topic 0건 시 제안된 근접 topic 후보
 */
function buildRecallHint(fragments, args, flags = {}, topicCandidates = []) {
  if (!fragments.length && Array.isArray(topicCandidates) && topicCandidates.length > 0) {
    const listed = topicCandidates.map(c => `${c.topic}(${c.count}건)`).join(", ");
    return {
      signal    : "topic_mismatch",
      suggestion: `topic '${args?.topic ?? ""}' 파편이 없습니다. 유사 토픽: ${listed}. topic=${topicCandidates[0].topic}으로 재검색하세요.`,
      trigger   : "recall"
    };
  }
  if (!fragments.length) {
