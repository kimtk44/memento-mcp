  ...
  inputSchema: {
    ...
    properties: {
      ...
      resolutionStatus: {
        type       : "string",
        enum       : ["open", "resolved", "abandoned"],
        description: "에러/케이스 파편의 해결 상태를 명시적으로 종결한다(reflect-by-id). " +
                       "resolved: 해결 완료(스케줄 GC drain 대상), abandoned: 방치 종결(컨텍스트 주입 제외, 가역). " +
                       "id로 특정 error 파편을 직접 close하는 1급 경로 — open error가 영구 주입되는 것을 막는다.",
        examples   : ["resolved", "abandoned", "open"]
      },
      agentId: COMMON_PARAMS.agentId,
      targetWorkspace: {
        type       : "string",
        description: "파편을 이전할 대상 워크스페이스. 명시 시 해당 workspace로 변경, " +
...
export const reflectDefinition = {
  ...
  description : "세션 종료 시 학습 내용을 원자 파편으로 영속화한다. " +
                 ...
                 "summary/narrative_summary 등 항목을 명시 전달하면 그대로 저장한다. " +
                 "consolidate=true 일 때만 해당 세션의 working-memory에서 미입력 항목을 자동 종합한다 " +
                 "(기본 false — 동시 세션 혼입·중복 방지). " +
                 "summary 또는 narrative_summary 중 하나 이상 권장. " +
  ...
  inputSchema: {
    ...
    properties: {
      ...
      consolidate: {
        type       : "boolean",
        description: "true일 때만 해당 세션의 working-memory에서 미입력 항목(summary/decisions/...)을 자동 종합한다. " +
