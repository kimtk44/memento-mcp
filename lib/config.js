...
/**
 * 임베딩 클라이언트 fail-fast 설정.
 * bge-m3가 llama-swap에서 evict되면 reload(60~120s)로 L3가 막힌다. 장시간 hang 대신
 * 짧은 timeout에 끊고 빈 결과를 반환(L1/L2는 계속 작동). 기본 10s / 재시도 0회.
 */
export const EMBEDDING_TIMEOUT_MS        = Number(process.env.EMBEDDING_TIMEOUT_MS || 10000);
