...
export class FragmentGC {
  ...
  async deleteExpired() {
    ...
    const result = await queryWithAgentVector("system",
      `WITH gc_candidates AS (
         ...
         WHERE ttl_tier NOT IN ('permanent')
           ...
           AND (
             ...
             OR
             (type = 'error'
              /* [fork-patch] key on resolution_status, not the '[해결됨]' prefix + importance<0.3:
               * reflect mints resolved errors at 0.5 and amend-resolved ones carry no prefix,
               * so the upstream predicate matches nothing here. */
             ...
             (source LIKE 'split:%'
              AND importance < $8
              AND access_count = 0
              AND (accessed_at IS NULL OR accessed_at < NOW() - make_interval(days => $9))
              AND created_at < NOW() - make_interval(days => $9)
             )
             OR
             (source LIKE 'split:%'
      ...
      [gracePeriodDays, utilityThreshold, inactiveDays, fdImportance, fdOrphanDays, erMaxDays, maxDelete, scMaxImportance, scOrphanDays, scTombstonedGrace],
      "write"
    );
  ...
  /**
   * [fork-patch] 오래된 open 에러의 전진 가드. resolved로 종결되지 않은 채 방치된
   * 저중요도 error 파편을 abandoned로 내려 컨텍스트 주입·recall 후보에서 빠지게 한다.
   * 이게 없으면 open 에러가 영구 누적된다(2026-06 error_count=90 사고의 절반).
   */
