...
/**
 * ON CONFLICT 시 relation_type 결정식.
 * 기존 링크가 superseded_by면 유지한다 — supersede 체인은 이력(fragment_history 등)이
 * 의존하는 관계이고, 이후 자동 링킹(GraphLinker/SessionLinker)의 related 등 약한 관계가
 * 덮어쓰면 만료 파편이 recall 링크 확장으로 재유입된다.
 */
const KEEP_SUPERSEDED_BY_SQL =
  `CASE WHEN ${SCHEMA}.fragment_links.relation_type = 'superseded_by'
        THEN ${SCHEMA}.fragment_links.relation_type
        ELSE EXCLUDED.relation_type END`;
...
export class LinkStore {
      ...
      await client.query(
        `INSERT INTO ${SCHEMA}.fragment_links (from_id, to_id, relation_type, weight)
               ...
               ON CONFLICT (from_id, to_id) DO UPDATE
                 SET relation_type = ${KEEP_SUPERSEDED_BY_SQL},
  ...
  async createLinks(pairs, agentId = "default") {
    ...
    const result = await withTransaction(pool, async (client) => {
      ...
      /**
       * multi-row INSERT 빌드.
       * fragment_links에는 accessed_at 컬럼이 없다(memory-schema.sql·마이그레이션 전체 미정의).
       * 과거 SET accessed_at 절 때문에 배치 INSERT가 매번 실패해 단건 fallback으로만 동작했다.
   ...
   * @param {boolean}     opts.includeSuperseded  - true면 만료(valid_to 설정) 파편도 포함 (기본 false)
   ...
   * @param {string|null} opts.workspace
   * @param {boolean}     opts.includePeerAgents
   * @param {boolean}     opts.includeSuperseded  - true면 만료(valid_to 설정) 파편도 포함
  ...
  _buildLinkedFragmentsSql({
    ...
    /** 만료(valid_to 설정) 파편은 includeSuperseded 요청 시에만 확장 대상 */
