...
export class FragmentIndex {
  ...
  /**
   * 만료(valid_to 설정)된 파편을 역인덱스와 Hot Cache에서 제거한다.
   *
   * supersede·dedup·모순 해소·분할 등 valid_to를 세팅하는 모든 경로가 호출해야 한다.
   * Hot Cache에는 만료 이전(valid_to=null) 스냅샷이 남아 있어, 제거하지 않으면
   * L1 → HotCache 경로로 만료 파편이 recall 결과에 재유입된다
   * (search()의 !f.valid_to 필터를 스냅샷이 통과하고, 매 recall마다 TTL이 갱신된다).
   *
   * Hot Cache 키는 조회자 keyId 네임스페이스로 기록되므로(마스터 키 조회 → "_g"),
   * key_id가 있는 파편은 "_g" Hot Cache 키도 함께 삭제한다.
   *
   * @param {Array<{id: string, keywords?: string[], topic?: string, type?: string, key_id?: *}>} rows
   *        - UPDATE ... RETURNING id, keywords, topic, type, key_id 결과 행
   */
  async deindexExpired(rows) {
    if (!rows || rows.length === 0) return;
    if (!redisClient || redisClient.status !== "ready") return;

    for (const r of rows) {
      const keyId = r.key_id ?? null;
      await this.deindex(r.id, r.keywords, r.topic, r.type, keyId);
      if (keyId != null) {
        await redisClient.del(`${HOT_PREFIX}_g:${r.id}`).catch(() => {});
      }
    }
  }
  ...
  async evictWorkingMemoryItems(sessionId, ids) {
    if (!redisClient || redisClient.status !== "ready" || !sessionId) return 0;
