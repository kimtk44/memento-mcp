...
async function rerankExternal(query, documents) {
  try {
    ...
    // llama.cpp --reranking (cohere shape): {results:[{index, relevance_score(raw logit)}]}.
    // Reindex to document order + sigmoid -> [0,1] (matches in-process convention for downstream recencyBoost).
    if (Array.isArray(data.results)) {
      ...
      for (const r of data.results) {
        ...
        if (Number.isInteger(idx) && idx >= 0 && idx < scores.length) {
          scores[idx] = 1 / (1 + Math.exp(-raw));
        }
      }
      return scores;
    }
    // generic external service: {scores:[...]} aligned to documents
