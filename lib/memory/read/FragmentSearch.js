...
export class FragmentSearch {
  ...
  async _searchL1(query, keyId = null) {
    ...
    if (sets.length === 0) {
      /** 명시 조건(keywords/topic/type/text)이 하나라도 있는 쿼리는 L1이 후보를
       *  지어내지 않는다. L2/L3가 SQL 필터로 담당한다. getRecent 폴백은 조건이
       *  전무한 최근 기억 조회 전용이다.
       *  isFallback: false — 조건부 miss가 폴백 메트릭을 오염시키지 않게 한다. */
      const hasExplicitPredicate = (query.keywords?.length > 0) || query.text;
  ...
  async _searchL2(query, excludeIds = [], agentId = "default", keyId = null, timeRange = null) {
    ...
    if (excludeIds.length > 0) {
      ...
      if (missingIds.length > 0) {
         ...
         *  파편이 최종 결과에 유입된다(superseded는 getByIds SQL이 valid_to로 차단). */
