...
/** 주어 앵커로 쓸 품사 — 고유명사(NNP)·외국어(SL)·한자(SH). */
const PROPER_NOUN_POS = new Set(["NNP", "SL", "SH"]);

/**
 * 한국어 형태소 분석 결과에서 고유명사·외국어·한자 표면형을 추출한다.
 * 분석기 로드나 분석이 실패하면 null을 반환해 호출부가 "판정 불가"를 구분할 수 있게 한다.
 *
 * @param {string} text
 * @returns {Promise<string[]|null>}
 */
export async function extractProperNounTokens(text) {
  ...
  try {
    ...
    return tokens
      .filter(t => PROPER_NOUN_POS.has(t?.pos))
      .map(t => t.text)
