...
export const recallDefinition = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      rankingMode: {
        type       : "string",
        enum       : ["balanced", "semantic"],
        description: "결과 정렬 모드. balanced(기본): importance+recency+similarity 복합 (메모리다운 최신 선호). " +
                       "semantic: recency-off 순수 의미 정렬 (rerankerScore 우선, 없으면 importance 0.1 + similarity 0.9). " +
                       "오래된 특정 사실/결정을 정확히 회상할 때 사용 — 최신 파편에 매몰되지 않음."
      },
      tokenBudget: {
        type       : "number",
...
export const amendDefinition = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      workspace: {
        type       : "string",
        description: "워크스페이스 변경. 명시 시 해당 workspace로 이전, 미명시 시 기존 workspace 보존."
      }
    },
    required: ["id"]
  }
