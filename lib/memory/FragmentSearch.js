...
export class FragmentSearch {
  ...
  async search(query) {
    ...
    /** 중복 제거 */
    const unique = this._deduplicate(combined, sq.fragmentCount, sq.anchorTime, sq.rankingMode);
  ...
  async _executeSearch(sq, _metricsP) {
    ...
    if (sq.text && EMBEDDING_ENABLED) {
       ...
       *  semantic 모드는 우회: ms-marco-MiniLM(영어 cross-encoder)이 한국어에서 ~uniform 점수라
       *  변별이 없고, 30→15 컷이 high-similarity 옛 파편을 누락시킴. 우회 시 RRF(=L3 cosine) 순서 유지. */
      if (sq.rankingMode !== "semantic" && isRerankerAvailable() && sq.text && combined.length > 0) {
   ...
   * @returns {Array}
   */
  _deduplicate(fragments, _fragmentCount = 0, anchorTime = Date.now(), rankingMode = null) {
    const seen = new Map();
    ...
    const semanticMode = rankingMode === "semantic";
    const scoreOf = (f) => {
      // semantic 모드(recency-off): L3 similarity 기준 우선 (rerankerScore는 한국어 무변별이라 미사용)
      if (semanticMode) return (f.similarity || 0) + (f.importance || 0) * 0.001;
      if (f.rerankerScore !== undefined) return f.rerankerScore;
      return this._computeRankScore(f, MEMORY_CONFIG, anchorTime);
    };
  ...
  _trimToTokenBudget(fragments, tokenBudget) {
    ...
    /** 최소 1건 보장: 최상위 파편이 예산을 초과해도 결과를 통째로 버리지 않는다.
     *  (예산 초과 1건 단독 starvation으로 recall 0건 회귀 방지) */
