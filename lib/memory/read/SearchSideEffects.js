...
export async function commitSearchSideEffects(query, sq, cleanResult, ctx) {
  ...
  /** topic 정확일치 필터의 0건은 임계값 학습 신호로 쓰지 않는다.
   *  오기 한 글자로도 전 계층이 동시에 0건이 되어 min_similarity 하향 압력만 남긴다.
   *  search_events 기록은 유지하고 adaptor 학습에서만 제외한다. */
  const topicFilterMiss = Boolean(String(query?.topic ?? "").trim()) && ctx.rawResultCount === 0;
