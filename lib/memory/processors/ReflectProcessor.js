...
export class ReflectProcessor {
  ...
  async process(params) {
    ...
    /** 0. [fork-patch] consolidate opt-in 시에만 세션 working-memory에서 미입력 항목 종합.
     *  기본 off (2026-06-28 fix): Memento 세션은 토큰-키라 동시 CC 대화가 한 세션을
     *  공유 → 무조건 종합 시 타 대화 파편 혼입(D2)·기영속 파편 중복(D3)·자기참조
     *  메타요약 오염(D1)을 유발. lazy auto-synthesis가 필요한 호출자만 명시 opt-in. */
    if (params.consolidate && params.sessionId) {
