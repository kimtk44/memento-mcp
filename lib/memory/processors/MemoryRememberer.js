...
const AMENDABLE_PARAMS = new Set([
  ...
  "agentId", "dryRun",
  "targetWorkspace"   // [fork-patch] 누락 시 단독 호출이 "amendable 필드 없음"으로 거부되고
                      //              unsupportedFields에도 잘못 실린다 (2026-08-14 redteam)
]);
...
export class MemoryRememberer {
  ...
  async _persistNonAtomic(fragment, params, agentId, keyId, groupKeyIds) {
    ...
    /** 명시적 대체는 임베딩 큐 적재(postProcessor) 전에 처리한다 — _applySupersedes 참조 */
    await this._applySupersedes(params, id, agentId, keyId);
   ...
   */
  /**
   * remember의 supersedes 목록을 처리한다(명시적 대체).
   *
   * 반드시 postProcessor.run(임베딩 큐 적재) 이전에 호출해야 한다. 이후에 처리하면
   * EmbeddingWorker → GraphLinker.linkFragment의 dedup 게이트(cos >= 0.95)가 아직 live인
   * 구 파편을 "원본"으로 보고 방금 만든 신규 파편을 만료시키는 경합이 생긴다
   * (2026-09-24 trunc-restore 15건 실측: 신규 valid_to가 ~1초 뒤 세팅, 구 파편 supersede는 그 이후).
   *
   * @param {Object}      params  - remember 원본 params
   * @param {string}      id      - 신규 파편 ID
   * @param {string}      agentId
   * @param {string|null} keyId
   */
  async _applySupersedes(params, id, agentId, keyId) {
    if (!params.supersedes || !Array.isArray(params.supersedes)) return;
  ...
  async amend(params) {
    ...
    if (params.targetWorkspace !== undefined)  updates.workspace        = params.targetWorkspace;  // [fork-patch]
  ...
  async forget(params) {
    ...
    if (params.topic) {
      const topicFrags = await this.store.searchByTopic(params.topic, {
        ...
        includeTtlTier   : true,   // permanent 보호 판정용 (미조회 시 force 없이도 삭제됐다)
        limit: 200,
      });
      ...
      /** M5 dryRun (topic 경로): 삭제 없이 삭제 예정 목록만 반환한다.
       *  id 경로만 dryRun을 처리하던 시절에는 topic+dryRun이 실제 삭제를 수행했다. */
      if (params.dryRun === true) {
        return {
          dryRun   : true,
          simulated: {
