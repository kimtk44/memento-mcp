...
/**
 * 임베딩 프로바이더 장애 시 등록 폭주를 막는 프로세스 내 가드.
 * - 쿨다운: 프로바이더 오류 감지 시 지수 백오프로 신규 등록을 중단한다.
 * - inFlight: 동시 recall이 같은 형태소를 중복 임베딩 요청하지 않게 한다.
 */
const EMBED_COOLDOWN_BASE_MS = 30_000;
const EMBED_COOLDOWN_MAX_MS  = 600_000;
let   embedCooldownUntil     = 0;
let   embedCooldownMs        = EMBED_COOLDOWN_BASE_MS;
const inFlightMorphemes      = new Set();

/** 프로바이더 오류 분류. permanent(인증·권한)와 transient(한도·서버·네트워크)만 쿨다운 대상이다. */
function classifyEmbedError(message) {
  const msg = String(message || "");
  if (/\b(401|403)\b|unauthorized|invalid[_ ]api[_ ]key|incorrect api key/i.test(msg)) {
    return "permanent";
  }
  if (/\b(429|5\d\d)\b|rate.?limit|timed?.?out|econn|enotfound|eai_again|socket|network/i.test(msg)) {
    return "transient";
  }
  return "other";
}

function activateEmbedCooldown(kind, message) {
  embedCooldownUntil = Date.now() + embedCooldownMs;
  logWarn(`[MorphemeIndex] 임베딩 프로바이더 오류(${kind}), ${Math.round(embedCooldownMs / 1000)}초 신규 등록 중단: ${message}`);
  embedCooldownMs = Math.min(embedCooldownMs * 2, EMBED_COOLDOWN_MAX_MS);
}

function resetEmbedCooldown() {
  embedCooldownMs    = EMBED_COOLDOWN_BASE_MS;
  embedCooldownUntil = 0;
}
...
export class MorphemeIndex {
   ...
   * - multi-row INSERT 1회: ON CONFLICT 시 NULL 행만 조건부 UPDATE(유효 임베딩 보존)
  ...
  async getOrRegisterEmbeddings(morphemes) {
    ...
    /** embedding IS NULL 행은 과거 계산 실패의 잔재이므로 히트가 아니라 miss로 취급한다.
     *  NULL을 히트로 간주하면 재계산 없이 영구 네거티브 캐시가 된다. */
    const existing = await pool.query(
      `SELECT morpheme, embedding::text FROM ${SCHEMA}.morpheme_dict
       WHERE morpheme = ANY($1) AND embedding IS NOT NULL`,
      [valid]
    );

    const found   = new Map(existing.rows.map(r => [r.morpheme, r.embedding]));
    let   missing = valid.filter(m => !found.has(m));

    /** 프로바이더 장애 쿨다운 중에는 신규 임베딩 시도를 생략하고(폭주 방지),
     *  다른 호출이 이미 등록 중인 형태소는 중복 API 호출을 만들지 않는다. */
    if (missing.length > 0 && Date.now() < embedCooldownUntil) {
      missing = [];
    }
    if (missing.length > 0) {
      missing = missing.filter(m => !inFlightMorphemes.has(m));
    }

    if (missing.length > 0) {
      for (const m of missing) inFlightMorphemes.add(m);
      try {
        await this._batchRegister(missing, found, pool);
      } finally {
        for (const m of missing) inFlightMorphemes.delete(m);
      }
    }
  ...
  async _registerChunk(batch, found, pool) {
    ...
    try {
      ...
      if (badIdxs.size > 0) {
        ...
        /** 프로바이더 계층 장애(인증·한도·서버·네트워크)는 단건 fallback으로 넘기지 않는다.
         *  배치 1회 실패가 단건 200회 + 클라이언트 재시도로 증폭되는 것을 차단한다. */
        const kind = classifyEmbedError(err.message);
        if (kind !== "other") {
          activateEmbedCooldown(kind, err.message);
          return;
        }
    ...
    try {
      /** NULL 행은 조건부 UPDATE로 치유한다. DO NOTHING이면 기존 NULL이 영구 잔존한다.
       *  유효 임베딩이 이미 있는 행은 덮어쓰지 않는다(프로바이더·차원 혼합 방지). */
      await pool.query(
        `INSERT INTO ${SCHEMA}.morpheme_dict (morpheme, embedding)
         VALUES ${valuePlaceholders.join(", ")}
         ON CONFLICT (morpheme) DO UPDATE SET embedding = EXCLUDED.embedding
         WHERE morpheme_dict.embedding IS NULL`,
        params
      );
      resetEmbedCooldown();
  ...
  async _fallbackSingleRegister(batch, found, pool) {
    for (const morpheme of batch) {
      try {
        ...
        /** 프로바이더 계층 장애면 남은 단건 시도를 중단하고 쿨다운으로 전환한다. */
        const kind = classifyEmbedError(err.message);
        if (kind !== "other") {
          activateEmbedCooldown(kind, err.message);
          return;
