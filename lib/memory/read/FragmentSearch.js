...
export class FragmentSearch {
  ...
  async _buildTextRRF(sq, l1Ids, l1MissIds, l1IsFallback, cached, agentId, keyId, timeRange, searchPath, layerLatency) {
    ...
    if (isRerankerAvailable() && sq.text && combined.length > 0 && process.env.MEMENTO_RERANK_OFF !== "1") {
  ...
  async _searchL3(query, agentId = "default", keyId = null, timeRange = null, scope = null) {
    try {
      ...
      if (process.env.MEMENTO_EVAL_DEBUG) {
        logInfo(`[EVAL_DEBUG] L3 minSim=${minSimilarity} limit=${limit ?? 10} ids(${results.length}): ${results.map(f => f.id).join(",")}`);
      }

      /** caseId/resolutionStatus/phase: SQL 미지원 필드는 scope로 정합 필터링 */
      if (scope && !scope.isNoop()) {
        const filtered = results.filter(f => scope.applyTo(f));
        if (process.env.MEMENTO_EVAL_DEBUG) logInfo(`[EVAL_DEBUG] L3 post-scope ids(${filtered.length}): ${filtered.map(f => f.id).join(",")}`);
        return filtered;
      }
      return results;
    } catch (err) {
  ...
  _computeRankScore(fragment, config, anchorTime = Date.now()) {
    ...
    /** env override (Phase 2 ranking-weight tuning; default = config or 0.4/0.3/0.3) */
    const importanceWeight    = process.env.MEMENTO_RANK_W_IMPORTANCE !== undefined ? Number(process.env.MEMENTO_RANK_W_IMPORTANCE) : (r.importanceWeight ?? 0.4);
    const recencyWeight       = process.env.MEMENTO_RANK_W_RECENCY    !== undefined ? Number(process.env.MEMENTO_RANK_W_RECENCY)    : (r.recencyWeight    ?? 0.3);
    const semanticWeight      = process.env.MEMENTO_RANK_W_SEMANTIC   !== undefined ? Number(process.env.MEMENTO_RANK_W_SEMANTIC)   : (r.semanticWeight   ?? 0.3);
    const recencyHalfLifeDays = r.recencyHalfLifeDays;
    ...
    return effectiveImp * importanceWeight
         + proximity    * recencyWeight
