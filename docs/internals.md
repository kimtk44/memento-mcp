...
18. `feedback_calibration` — 최근 24시간 피드백 기반 importance 점진 보정. `sufficient=true` 시 +5%, `sufficient=false` 시 −2.5%, `relevant=false` 시 −5%. lr=0.05, 클리핑 [0.05, 1.0]. `is_anchor=true` 제외
...
Gemini 계열 provider는 코딩 임무 위임 금지(CLAUDE.md 정책).
