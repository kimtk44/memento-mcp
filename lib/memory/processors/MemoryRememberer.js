...
const AMENDABLE_PARAMS = new Set([
  ...
  "agentId", "dryRun",
  "targetWorkspace"  // [fork-patch] amend로 파편 workspace 이전
]);
...
export class MemoryRememberer {
  ...
  /**
   * PolicyRules 게이트 평가 + workspace 허가 집합 검증.
   *
   ...
   *
   * SYMBOLIC_CONFIG.policyRules=false이면 PolicyRules 위반은 생략된다(no-op).
   * ApiKeyStore.checkWorkspaceAllowed는 symbolic 게이트와 무관하게 항상 평가하며,
   * workspaceNotAllowed 위반은 저장을 차단하지 않고 경고로만 남긴다.
   * fragmentHasWorkspace 위반은 환경변수 MEMENTO_WORKSPACE_GATE=true일 때만 hard gate 대상에 포함된다.
  ...
  async _runPolicyGate(fragment, { keyId, mode }) {
    ...
    let workspaceViolation = null;
    try {
      workspaceViolation = await checkWorkspaceAllowed(keyId, fragment.workspace);
    } catch (err) {
      /**
       * 판정 자체가 실패한 경우다. workspace를 주장한 저장을 통과시키면 격리가
       * 풀리므로 위반으로 다룬다. workspace 미기입 요청은 애초에 판정 대상이
       * 아니라 여기까지 오지 않는다.
       */
      logError(`[MemoryRememberer] workspace 허가 판정 실패: ${err.message}`);
    ...
    if (workspaceViolation) violations = [...violations, workspaceViolation];
     ...
     * hard gate 대상 필터.
     *
     * workspaceLookupFailed는 허가 여부를 판정하지 못한 상태이므로 항상 차단한다.
     * 모르는 채로 통과시키면 격리가 없는 것과 같다.
     *
     * workspaceNotAllowed와 fragmentHasWorkspace는 판정은 됐고 정책 선택의
     * 문제다. 기존 배포의 동작을 바꾸지 않도록 MEMENTO_WORKSPACE_GATE=true일
     * 때만 저장을 거부하고, 그 외에는 종전대로 경고로 남긴다.
     */
    const workspaceGateEnabled = process.env.MEMENTO_WORKSPACE_GATE === "true";
    const gateEligible = violations.filter(v => {
      if (v.rule === "workspaceLookupFailed") return true;
      if (v.rule === "workspaceNotAllowed")   return workspaceGateEnabled;
      if (v.rule === "fragmentHasWorkspace")  return workspaceGateEnabled;
      return true;
    });

    if (keyId != null && gateEligible.length > 0) {
      let hardGate;
      try {
      ...
      if (hardGate) {
        symbolicMetrics.recordGateBlock("policy", gateEligible[0]?.rule ?? "unknown");
        throw new SymbolicPolicyViolationError(gateEligible, {
  ...
  async amend(params) {
    ...
    if (params.dryRun === true) {
      ...
      if (params.targetWorkspace !== undefined) wouldBe.workspace = params.targetWorkspace;  // [fork-patch]
    ...
    /** [fork-patch] workspace 이전. 출처 컬럼(migration-040)도 명시 기입으로 갱신한다. */
    if (params.targetWorkspace !== undefined) {
      updates.workspace        = params.targetWorkspace;
      updates.workspace_source = "explicit";
    }
  ...
  async forget(params) {
    ...
    if (params.id) {
      ...
      if (!frag) {
        ...
        const probe = typeof this.store.probeAccess === "function"
          ...
          : null;
        /** 판정 불가면 종전 동작을 유지한다. 모르는 것을 성공으로 답하지 않는다. */
        if (!probe) {
    ...
    if (params.topic) {
      const topicFrags = await this.store.searchByTopic(params.topic, {
        ...
        includeTtlTier   : true,   // [fork-patch] permanent 보호 판정용 (미조회 시 force 없이도 삭제됐다)
        limit: 200,
      });
      ...
      /** [fork-patch] M5 dryRun (topic 경로): 삭제 없이 삭제 예정 목록만 반환한다.
       *  id 경로만 dryRun을 처리해 topic+dryRun이 실제 삭제를 수행했다. */
      if (params.dryRun === true) {
        return {
          dryRun   : true,
          simulated: {
            ...
            would_delete      : toDelete.map(f => ({
              id: f.id, type: f.type, ttl_tier: f.ttl_tier, valid_to: f.valid_to ?? null
            })),
