 ...
 * 복합 필터: INTERSECTION(교집합) 적용. topic/type 인덱스가 0건이면 L1은 공집합을 반환하고,
 * getRecent fallback은 명시 조건(keywords/topic/type/text)이 전무한 조회에만 발동
...
import { resolveQueryProfile, applySimilarityDelta, applyProfileToConfig, resolveMorphemeThreshold } from "./QueryProfile.js";
import { searchSyntheticQueries, isSyntheticSearchEnabled } from "./SyntheticQuerySearch.js";
...
import { SearchScope, normalizeIsAnchor } from "./SearchScope.js";
...
import { byDescendingScore, compareRankedFragments } from "./DeterministicRanking.js";
...
/**
 * workspace 스코프 랭킹 감쇠 배율을 계산한다.
 *
 * 검색 scope에 workspace가 지정된 경우, fragment.workspace가 scope와
 * 불일치하거나 NULL(전역)이면 penalty 배율을 반환한다. 반환 자체는
 * 유지하되(SearchScope 필터 의미론은 그대로) 랭킹 점수만 하향한다.
 * recall 최종 정렬(MemoryRecaller)과 context 주입 정렬(ContextBuilder)이
 * 이 단일 구현을 공유하여 이중 감쇠를 피한다.
 *
 * @param {Object|null} fragment
 * @param {string|null} workspace - 검색 scope workspace (미지정이면 감쇠 없음)
 * @returns {number} 1(감쇠 없음) 또는 penalty 배율
 */
export function computeWorkspaceDecayFactor(fragment, workspace) {
  if (!MEMORY_CONFIG.workspaceDecay?.enabled) return 1;
  if (workspace == null) return 1;
  if (fragment?.workspace === workspace) return 1;
  return MEMORY_CONFIG.workspaceDecay.penalty ?? 0.7;
}
...
export class FragmentSearch {
  ...
  async search(query) {
    ...
    /** 모든 후보 경로의 최종 공통 정합 필터. SQL/Redis 사전 필터는 성능용이며,
     *  hydration·graph·보조 검색이 필터를 우회하지 못하도록 반환 후보를 다시 검증한다. */
    const finalScope     = SearchScope.fromQuery(sq);
    const scopedCombined = finalScope.isNoop()
      ? combined
      : combined.filter(fragment => finalScope.applyTo(fragment));

    /** 중복 제거 */
    const unique = this._deduplicate(scopedCombined, sq.fragmentCount, sq.anchorTime, sq._profile);
    ...
    let clean = trimmed.map(({ _rrfScore, _kwExact, _kwSupplement, _l1Hydrated, ...rest }) => rest);
    ...
    if (clean.length > 0) {
      ...
      this.store.touchLinked(accessIds, sq.agentId, sq.keyId, {
  ...
  _buildSearchQuery(query) {
    const sq = {
      ...
      isAnchor          : normalizeIsAnchor(query.isAnchor),
      /** topic 앞뒤 공백 정규화 — 공백만이면 필터 미지정으로 강등(대소문자는 보존). */
      topic             : typeof query.topic === "string"
    ...
    sq._profile = resolveQueryProfile(query);
  ...
  async _buildTextRRF(sq, l1Ids, l1MissIds, l1IsFallback, cached, agentId, keyId, timeRange, searchPath, layerLatency) {
    ...
    // HotCache 파편을 DB 결과와 같은 hydration tail로 정규화해 cold/warm 순서를 일치시킨다.
    const hydratedL2 = mergeHydratedCandidates(l2Results, cached);
    ...
    const rrfLayers = [
      { name: "l1",    results: l1Ids,                    weightFactor: l1IsFallback ? 0.5 : MEMORY_CONFIG.rrfSearch.l1WeightFactor, unranked: !l1IsFallback },
      { name: "l2",    results: hydratedL2,                weightFactor: sq._profile?.l2WeightFactor ?? 1.0 },
      { name: "graph", results: graphResults,              weightFactor: MEMORY_CONFIG.rrfSearch.graphWeightFactor },
      { name: "l3",    results: l3Results,                 weightFactor: sq._profile?.l3WeightFactor ?? 1.0 },
    ];
    if (temporalResults.length > 0) {
    ...
    if (isRerankerAvailable() && sq.text && combined.length > 0) {
      ...
      const reranked      = await rerank(rerankQuery, rerankerInput, 15, sq.anchorTime).catch(() => null);
  ...
  async _buildFallbackCombined(sq, l1MissIds, cached, agentId, keyId, timeRange, searchPath, layerLatency) {
    ...
    const combined = tagExact(mergeHydratedCandidates(l2Results, cached));
    if (temporalResults.length > 0) {
      searchPath.push(`Temporal:${temporalResults.length}`);
  ...
  async _searchL1(query, keyId = null) {
    ...
    /** topic/type은 정확일치 하드 필터 계약이다. 지정된 필터의 인덱스 결과가 0건이면
     *  교집합은 공집합이므로 다른 집합으로 후보를 만들지 않고 즉시 빈 결과를 반환한다.
     *  0건 집합을 누락시키면 keyword 집합 단독이 교집합 행세를 하며 필터가 소실된다. */
    if (query.topic) {
    ...
    if (sets.length === 0) {
      /** isAnchor 단독 조회는 recent ID를 캐시 후보로 쓰되 일반 fallback으로
       *  표시하지 않는다. 캐시 miss ID는 L2 hydration으로 이어지고, L2는 별도의
       *  DB 필터 조회도 수행하므로 cold/warm 여부가 결과 의미를 바꾸지 않는다. */
      if (
        query.isAnchor !== undefined &&
        !(query.keywords?.length > 0) &&
        !query.topic &&
        !query.type &&
        !query.text
      ) {
        const ids = await this.index.getRecent(30, keyId);
        return { ids, isFallback: false };
      }

      /** 명시 조건(keywords/topic/type/text/isAnchor)이 하나라도 있는 쿼리는 L1이 후보를
       *  지어내지 않는다. L2/L3가 SQL 필터로 담당한다. getRecent 폴백은 조건이
       *  전무한 최근 기억 조회 전용이다.
       *  isFallback: false — 조건부 miss가 폴백 메트릭을 오염시키지 않게 한다. */
      const hasExplicitPredicate = (query.keywords?.length > 0) || query.text || query.isAnchor !== undefined;
  ...
  async _searchL2(query, excludeIds = [], agentId = "default", keyId = null, timeRange = null) {
    ...
    /** 조건 없는 최근 기억 조회와 isAnchor-only/type 조합은 시간 범위 reader를
     *  메타데이터 조회 경로로 재사용한다. L1 상위 ID가 다른 workspace로 가득 차도
     *  SQL workspace 조건으로 허용 범위의 최근 파편을 다시 채운다. */
    const hasKeywords = Array.isArray(query.keywords) && query.keywords.length > 0;
    if (
      results.length === 0 &&
      !hasKeywords &&
      !query.topic &&
      ...
      results = await this.store.searchByTimeRange(
        timeRange?.from ?? null,
        timeRange?.to ?? null,
        {
          ...options,
          ...(query.isAnchor !== undefined ? { orderByImportance: true } : {})
        }
      );
    }
    ...
    if (excludeIds.length > 0) {
      ...
      if (missingIds.length > 0) {
        ...
        /** ID 보충 조회는 SQL 필터를 우회하므로 SearchScope로 전량 정합 필터링한다.
         *  topic/type/workspace/caseId/phase/affect가 여기서 소실되면 L1 유래 무관
         *  파편이 최종 결과에 유입된다. superseded 포함 여부는 getByIds에도 전달한다. */
        const hydrationScope = SearchScope.fromQuery(query);
        if (!hydrationScope.isNoop()) {
          fetched = fetched.filter(f => hydrationScope.applyTo(f));
        }
        if (timeRange?.from) {
          fetched = fetched.filter(f => f.created_at && new Date(f.created_at) >= new Date(timeRange.from));
        }
        if (timeRange?.to) {
          fetched = fetched.filter(f => f.created_at && new Date(f.created_at) <= new Date(timeRange.to));
        }
        results.push(...fetched.map(f => ({ ...f, _l1Hydrated: true })));
      }
    }
  ...
  async _searchL3(query, agentId = "default", keyId = null, timeRange = null, scope = null) {
    try {
      ...
      const baseSim       = adaptedSim ?? cfgSim ?? 0.35;
      const minSimilarity = applySimilarityDelta(baseSim, query._profile);
      ...
      const morphThresh   = resolveMorphemeThreshold(morphCfg.fallbackThreshold ?? 5, query._profile);
      ...
      /** 보조 역질의 프로브. 본 검색과 병렬로 띄워 채택 여부가 지연에 영향을 주지 않게 한다.
       *  순차 실행하면 매 검색에 벡터 조회가 하나 더 붙어 p95가 배가 된다. */
      const syntheticProbe = isSyntheticSearchEnabled()
        ? searchSyntheticQueries(vec, {
            agentId,
            keyId,
            groupKeyIds: Array.isArray(keyId) ? keyId : undefined,
            workspace: query.workspace ?? null,
            allWorkspaces: query.allWorkspaces === true,
            minSimilarity,
            includeSuperseded: query.includeSuperseded === true,
      ...
      const [results, morphemeResults, syntheticResults] = await Promise.all([
      ...
      /** 보조 역질의 결과 병합.
       *  본문 임베딩이 표기 불일치로 놓친 파편을 회수한다. 이미 본문 경로로 찾은
       *  파편은 건너뛰고, 채택 상한까지만 합류시켜 본문 결과를 밀어내지 않게 한다. */
      if (syntheticResults.length > 0) {
        const seenIds  = new Set(results.map(f => f.id));
        const adoptCap = MEMORY_CONFIG.syntheticQuery?.adoptLimit ?? 5;
        let   adopted  = 0;

        for (const frag of syntheticResults) {
          if (adopted >= adoptCap) break;
          if (seenIds.has(frag.id)) continue;
          results.push(frag);
          seenIds.add(frag.id);
          adopted++;
        }
  ...
  _computeRankScore(fragment, config, anchorTime = Date.now()) {
    ...
    const createdAt = Number.isFinite(parsed) ? parsed : anchorTime;
   ...
   * @returns {Array}
   */
  _deduplicate(fragments, _fragmentCount = 0, anchorTime = Date.now(), profile = null) {
    const seen = new Map();
    ...
    const effectiveConfig = applyProfileToConfig(MEMORY_CONFIG, profile);
    const scoreOf = (f) => f.rerankerScore !== undefined
      ? f.rerankerScore
      : this._computeRankScore(f, effectiveConfig, anchorTime);

    return allFragments.sort(byDescendingScore(scoreOf));
  }
    ...
    while (remaining.length > 0 && selected.length < withEmb.length) {
      ...
      for (let i = 0; i < remaining.length; i++) {
        ...
        if (mmrScore > bestScore || (
          mmrScore === bestScore && bestIdx >= 0 &&
          compareRankedFragments(candidate, remaining[bestIdx], () => mmrScore) < 0
        )) {
          bestScore = mmrScore;
          bestIdx   = i;
        }
  ...
  _trimToTokenBudget(fragments, tokenBudget) {
    ...
    if (!hasTags) {
      ...
      for (const f of fragments) {
        const c = cost(f);
        /** [fork-patch] 예산 초과 head를 통째로 버리지 않고 skip 후 더 작은 후보를 계속
         *  패킹한다. break면 뒤따르는 작은 파편이 전부 누락돼 recall 0건으로 회귀한다. */
        if (usedTokens + c > tokenBudget) continue;
        usedTokens += c;
        result.push(f);
      }
      /** [fork-patch] 최소 1건 보장: 모든 후보가 예산을 넘겨 비었으면 가장 작은 1건만
       *  admit한다. over-budget giant 강제 admit은 피하면서 recall 0건도 막는다. */
      if (result.length === 0 && fragments.length > 0) {
        let smallest = fragments[0];
        let smallestCost = cost(smallest);
        for (const f of fragments) {
          const c = cost(f);
          if (c < smallestCost) { smallest = f; smallestCost = c; }
        }
