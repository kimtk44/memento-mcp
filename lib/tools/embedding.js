...
function getOpenAIClient() {
  if (!openaiClient) {
    ...
    // Fail-fast: bge-m3가 evict되면 reload로 L3가 막힘. 장시간 hang 대신 짧게 끊고
    // 빈 결과 반환(L1/L2는 계속 작동). timeout/retries는 env 설정(lib/config.js).
    openaiClient  = new OpenAI({ ...options, timeout: EMBEDDING_TIMEOUT_MS, maxRetries: EMBEDDING_MAX_RETRIES });
  }
  return openaiClient;
}
 ...
 * transformers provider는 개별 embed 호출로 처리한다.
 *
 * @param {string[]} texts      - 임베딩할 텍스트 배열
 * @param {number}   batchSize  - API 호출당 처리 단위 (기본 100, transformers는 무시)
...
export async function generateBatchEmbeddings(texts, batchSize = 100) {
  if (EMBEDDING_PROVIDER === "transformers") {
    ...
    for (const text of texts) {
      allVectors.push(await embedder.embed(text));
    }
    return allVectors;
  }
