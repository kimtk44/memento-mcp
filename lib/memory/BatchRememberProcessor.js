...
export class BatchRememberProcessor {
  ...
  async process(params, onProgress = null) {
    ...
    try {
      ...
      for (const { fragment } of validFragments) {
        ...
        this._registerMorphemes(results[idx].id, fragment.content);
      }
  ...
  /**
   * 형태소 사전 등록 + morpheme_indexed 마킹 (fire-and-forget, Phase C 후처리).
   *
   * RememberPostProcessor Phase 4와 동일 semantics: 등록 체인 완료 시에만
   * morpheme_indexed=true. 단건 remember 경로만 마킹하고 batch/reflect 경로
   * (reflect는 Phase 1 이후 batch에 위임)가 마킹을 누락해 2026-06-16부터
   * morpheme_indexed=false 백로그가 쌓이던 결함의 fix (2026-07-17).
   * 실패는 warn만 — 미인덱싱 파편은 L3 morpheme leg에서만 제외되는 soft 손실.
   */
  _registerMorphemes(fragmentId, content) {
    morphemeIndex.tokenize(content ?? "")
      .catch(() => [])
      .then(morphemes => morphemeIndex.getOrRegisterEmbeddings(morphemes))
      .then(() => this._getPool().query(
        `UPDATE ${SCHEMA}.fragments SET morpheme_indexed = true WHERE id = $1`,
        [fragmentId]
      ))
      .catch(err => {
        logWarn(`[BatchRememberProcessor] morpheme registration failed (id=${fragmentId}): ${err.message}`);
      });
  }
